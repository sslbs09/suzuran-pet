...
### 块 27（用户：桌宠在左边老是闪现，不是很影响）
- 根因：左缘翻边每次窗口瞬移 ±(宽-124) 就是一下闪现；边界附近来回翻边=反复闪。原有死区8px+防抖500ms 仍偏敏感
...
### 块 32（用户报：左缘"光走路不前进"）
- 根因：左缘出屏钳回（charLeft<edgeL overdue）只 setPosition 贴边、不改行走方向，dir 仍向左→每 tick 向左推又被钳=原地踏步
