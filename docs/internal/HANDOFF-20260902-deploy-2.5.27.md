...
## 五点二十、用户三项排查：离线语音/自动更新/桌面位置识别（8408e03，已部署 12:57）

### 查1：完全预加载后开离线模式会不会回退系统音 —— **正常路径不会，已堵一个旁路缺口**
- 实测本机缓存：单指纹目录 356/358 ready（仅 2 句预加载失败）。
- 离线模式开关（pet:set-fixed-only）只改 tts.fixedOnly + 杀引擎进程，**不改引擎配置** →
  语音指纹不变 → 已预加载句子全部按 lineId 直查命中，不出系统音。
- 会出系统音的仅：预加载失败的 2 句、不在台词池的动态内容（聊天回复/日程提醒等，设计如此）。
- 修复旁路 bug：get-state 的 ttsCloud.enabled（渲染层"是否调 speakClone"的总门）不含
  ttsGsv——genie/cosy/cloud 全关只开 GSV 的用户会被直接判系统语音，永不进克隆链路。
- 提醒：不要用"语音设置里关 Genie/GSV 引擎开关"代替离线模式开关——那会改指纹让已
  预加载缓存全部失效（设计使然：指纹表达音色来源）。

### 查2：自动更新是否真实有效 —— 发现"他人装新版还是旧的"漏洞并修复
- 铁证：release.yml `npm run dist` 用 **--asar=false**，zip 里没有 app.asar，最终用户首次
  运行的是 resources/app 散目录；Electron 加载顺序 {app.asar, app}（node_bindings.cc 源码
  佐证）→ 首次自动更新后 asar 永远压过散目录。
- 漏洞：此后再用新版 zip 覆盖解压（散目录=新版、旧 asar 仍在）→ 继续跑旧 asar →
  "装了新版还是旧的"。修复：启动时散目录版本 > 运行版本 → 退出后删旧 asar 重启加载新版
  （marker 防重启循环，版本追平自动清理）。
- 另修两处：检查失败（GitHub 不可达）此前误报"已是最新"→ 现在明确报"无法连接更新服务器"
  （checkForUpdateDetailed，15s 超时）；applyOnExit 写脚本失败（安装目录不可写）此前被静默
  吞掉 → 现在弹明确错误。i18n 三语补齐。
- 遗留建议（不动）：dist 改 --asar=true + unpack 原生模块可从根上统一模型，但涉及 koffi/
  scripts 解包验证，宜单独立项。

### 查3：桌面应用位置识别 —— 用户指的选项没开且确实没跑；顺带收窄屏障扫描
- 设置页「桌面图标感知」（features.desktopIcons）有独立授权门控，默认关，未开启时
  listDesktopIcons 直接短路 → 用户没开就没跑 ✓。
- 真正常驻的是「窗口屏障」扫描（koffi EnumWindows 读全机可见窗口位置/标题，服务"站上
  其他窗口"与防遮挡）：此前每 3s 无条件轮询+每轮写日志。现改为与行走引擎联动（关行走/
  隐藏托盘时跳过），日志只在数量变化时记。

验证：42/42；部署重启 5 进程、相位轮换/预热正常、屏障日志不再刷屏。
