...
- **隐藏即静默待命**：托盘隐藏后不会主动搭话、不会出声（自己设置的提醒/番茄钟等任务到点照常提醒）
- **多皮肤切换**：托盘「🧩 小人皮肤」子菜单一键切换；在 `resources\app\renderer\spine\user\` 下每个皮肤建一个文件夹放入模型文件即可被识别（如 `summer/`、`winter/`）；文件名避免 `#` 等特殊符号
