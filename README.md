...
- **桌面行走**：设置里勾选「🚶 桌面行走」（或托盘菜单开关）。小人以**任务栏上沿为地面**左右散步、走走停停（明日方舟基建节奏），偶尔**跳到桌面上的程序窗口顶上坐着歇一会**再跳回来；走路朝向跟随实际移动方向；拖拽她时暂停，单击她播放互动动作
- **隐藏即静默待命**：托盘隐藏后不会主动搭话、不会出声（自己设置的提醒/番茄钟等任务到点照常提醒）
- **懒人换自己的小人**：把任意 Spine 模型的 `xxx.atlas` + `xxx.skel`（或 `.json`）+ `xxx.png` 放进 `resources\app\renderer\spine\user\` 文件夹，重启桌宠即自动替换，无需任何配置；删掉即恢复内置苏苏洛
