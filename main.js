...
function startAgentApi() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      const validClients = sanitizeClients(rawClients);
      const needsAuth = !!master || validClients.length > 0;
      ...
      if (clientHit) { // 接入名单：记录该 agent 最近活跃（在线状态展示）
        ...
        config.saveConfig({ agentApi: { ...apiCfg, clients: rawClients } });
      }
...
function sendProactive(text, emotion, { force = false } = {}) {
  ...
  const gate = lineGate.admit(text, { force });
  if (!gate.accepted) {
    logTts("lines", "跳过台词: " + gate.reason);
    return false;
  }
  sendToRenderer("pet:proactive", { text, emotion, force });
  return true;
}
  ...
  wsWatchTimer = setInterval(() => {
    try {
      ...
      if (sig !== wsWatchSig) {
        wsWatchSig = sig;
        if (wsWatchThrottle()) {
          emitWorkflowSignal("workspace");
          logTts("watch", "工作区活动已加入待发送信号");
        }
      }
    } catch { /* 忽略 */ }
  }, 5000);
...
ipcMain.handle("pet:save-settings", (_e, patch) => {
  ...
  try {
    ...
    const nextAgent = patch.agentApi ? { ...oldAgent, ...patch.agentApi } : oldAgent;
    config.saveConfig(patch);
    if (patch.agentApi && (oldAgent.enabled !== nextAgent.enabled || Number(oldAgent.port) !== Number(nextAgent.port))) {
