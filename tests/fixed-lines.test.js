...
const actionLine = manifest.find((i) => i.text.startsWith("（耳朵轻轻抖了抖）"));
assert.ok(actionLine);
assert.strictEqual(fixed.findItemNormalized({ name: "小苏", user: "阿明" }, actionLine.text.replace(/^（[^）]+）/, "")).id, actionLine.id);
const patLine = manifest.find((i) => i.id === "pat.08");
assert.ok(patLine);
assert.strictEqual(fixed.findItemNormalized({ name: "小苏", user: "阿明" }, "唔……阿明的手好温柔，再摸摸嘛").id, "pat.08");
